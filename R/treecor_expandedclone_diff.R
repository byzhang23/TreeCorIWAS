#' Identification of expanded clones and differential analysis
#'
...
#' \item expanded.clone: identified expanded clone in each cell type for each sample
#'}
#' @export
...
treecor_expandedclone_diff <- function(immunarch_list, chain, type = 'count', match = "clonotype", threshold = 2, aggregate = 'sample',
    ...
    ## =============================== ##
    if(verbose){
        message('Identify expanded clones...')
    }
    ...
    # get expanded clones at each node
    if(verbose) message('Get expanded clones at each node ...')
    exp.ls <- parallel::mclapply(1:length(dat.ls),function(i){
        if(verbose) message(i)
        getExpandedClone(dat.ls[[i]], match, type, threshold, aggregate, ncores) # check if this ncores will cause issue
    },mc.cores = ncores)
    names(exp.ls) <- names(dat.ls)

    detail.ls <- lapply(exp.ls,function(dt) dt[[2]])
    # names(detail.ls) <- suppressMessages(inner_join(data.frame(id = as.numeric(names(detail.ls))),
    #                                                 label_info)) %>% select(label) %>% unlist

    summary.ls <- lapply(exp.ls,function(dt) dt[[1]])
    pb.ls <- lapply(summary.ls,function(x){
        d <- as.matrix(x[,'expanded_prop',drop = F])
        rownames(d) <- x$sample
        d
    })
    ...
    final <- append(res,list(expanded.clone = detail.ls))
